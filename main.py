...
def add_topcoder_users(item:schema.Item):
    mydb = mysql.connector.connect(host=host_name, port=port_number, user=user_name, password=password_db, database=database_name)  
    ...
    cursor = mydb.cursor()
    sql = "INSERT INTO topcoder_users (nombres, apellidos, ciudad, correo, tc_name) VALUES (%s, %s, %s, %s, %s)"
    val = (nombres, apellidos, ciudad, correo, tc_name)
...
def update_topcoder_users(id:int, item:schema.Item):
    mydb = mysql.connector.connect(host=host_name, port=port_number, user=user_name, password=password_db, database=database_name)  
    nombres = item.nombres
    apellidos = item.apellidos
    ciudad = item.ciudad
    ...
    tc_name = item.tc_name
    cursor = mydb.cursor()
    sql = "UPDATE topcoder_users set nombres=%s, apellidos=%s, ciudad=%s, correo=%s , tc_name=%s  where id=%s"
    val = (nombres, apellidos, ciudad, correo, tc_name, id)
    cursor.execute(sql, val)
    mydb.commit()
    mydb.close()
